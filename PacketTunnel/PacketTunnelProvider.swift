...
class PacketTunnelProvider: NEPacketTunnelProvider, TunnelDelegate, ClientTunnelConnectionDelegate {
	...
	func createTunnelSettingsFromConfiguration(_ configuration: [NSObject: AnyObject]) -> NEPacketTunnelNetworkSettings? {
		guard let tunnelAddress = tunnel?.remoteHost,
			let address = getValueFromPlist(configuration, keyArray: [SettingsKey.IPv4, .Address]) as? String,
			let netmask = getValueFromPlist(configuration, keyArray: [SettingsKey.IPv4, .Netmask]) as? String
			else { return nil }
		...
		if let routes = getValueFromPlist(configuration, keyArray: [SettingsKey.IPv4, .Routes]) as? [[String: AnyObject]] {
